...
async function testRingCentralJWT() {
  ...
  try {
    ...
    // Probar acceso a la cuenta usando ~ (sin account_id)
    ...
    if (phoneNumber) {
      ...
      try {
        ...
        console.log('  From:', smsResponse.data.from?.phoneNumber || 'N/A');
        console.log('  To:', smsResponse.data.to?.[0]?.phoneNumber || 'N/A');
        console.log('');
        ...
        if (smsError.response) {
          ...
          console.log('  Error Code:', smsError.response.data?.errorCode || 'N/A');
          console.log('  Message:', smsError.response.data?.message || JSON.stringify(smsError.response.data));
        } else {
