...
export class PinService {
    ...
    async setupPin(userId: string, pin: string) {
        ...
        if (!/^[A-Z0-9]{6}$/.test(pin)) {
            throw new BadRequestException('PIN must be exactly 6 alphanumeric characters.');
        }
        ...
        try {
            await this.emailService.sendPinActivatedEmail(user.email, user.name || user.firstName);
        } catch (e) {
    ...
    async changePin(userId: string, oldPin: string, newPin: string) {
        if (!/^[A-Z0-9]{6}$/.test(newPin)) {
            throw new BadRequestException('New PIN must be 6 alphanumeric characters.');
        }
