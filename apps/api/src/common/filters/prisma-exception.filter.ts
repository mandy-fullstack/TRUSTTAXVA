  ...
  private isPrismaError(exception: any): boolean {
    return (
      (exception?.name?.includes('Prisma') &&
        !exception?.name?.includes('PrismaClientKnownRequestError') &&
        !exception?.name?.includes('PrismaClientUnknownRequestError') &&
        !exception?.name?.includes('PrismaClientRustPanicError') &&
        !exception?.name?.includes('PrismaClientInitializationError') &&
        !exception?.name?.includes('PrismaClientValidationError')) === false ||
  ...
  private handlePrismaError(exception: any, response: Response, request: any) {
    ...
    const errorMap: Record<string, { status: number; message: string }> = {
      ...
      P1003: {
        status: HttpStatus.SERVICE_UNAVAILABLE,
        message:
          'Database is not available. Please contact support if this persists.',
      },
      P1008: {
        status: HttpStatus.GATEWAY_TIMEOUT,
