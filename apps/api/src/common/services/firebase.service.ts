  ...
  onModuleInit() {
    if (admin.apps.length === 0) {
      ...
      try {
        ...
        if (!jsonStr) {
          console.warn(
            '[FirebaseService] FIREBASE_SERVICE_ACCOUNT_JSON is not defined.',
          );
        } else {
          ...
          try {
            ...
            console.log(
              '[FirebaseService] Firebase Admin initialized with Service Account successfully.',
            );
            return; // Done
          } catch (parseError: any) {
            console.error(
              '[FirebaseService] Failed to parse FIREBASE_SERVICE_ACCOUNT_JSON:',
              parseError.message,
            );
        ...
        if (process.env.FIREBASE_PROJECT_ID) {
          ...
          this.isInitialized = true;
          console.log(
            '[FirebaseService] Firebase Admin initialized with Project ID fallback:',
            process.env.FIREBASE_PROJECT_ID,
          );
        } else {
          console.warn(
            '[FirebaseService] CRITICAL: No Firebase configuration found (JSON or Project ID).',
          );
        }
      } catch (error: any) {
        console.error(
          '[FirebaseService] Unexpected error during initialization:',
          error,
        );
      }
    } else {
      this.isInitialized = true;
      console.log(
        '[FirebaseService] Firebase Admin already initialized (Apps count: ' +
          admin.apps.length +
          ')',
      );
    }
