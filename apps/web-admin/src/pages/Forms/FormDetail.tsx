...
export function FormDetailPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const di = data.descriptionI18n as
        | { en?: string; es?: string }
        | undefined;
      setMeta({
        name: data.name ?? "",
  ...
  const saveMeta = async (
    ...
    if (next.nameEn || next.nameEs)
      payload.nameI18n = {
        en: next.nameEn || undefined,
        es: next.nameEs || undefined,
      };
    if (next.descriptionEn || next.descriptionEs)
      payload.descriptionI18n = {
        en: next.descriptionEn || undefined,
        es: next.descriptionEs || undefined,
      };
    try {
      ...
      setAlertDialog({
        isOpen: true,
        title: "Error",
        message: (e as Error)?.message || "Failed to save",
        variant: "error",
      });
    } finally {
      setSaving(false);
  ...
  const addSection = async () => {
    const t =
      sectionTitle.trim() || sectionTitleEn.trim() || sectionTitleEs.trim();
    if (!id || !t) return;
    try {
      const payload: any = { title: t };
      if (sectionTitleEn.trim() || sectionTitleEs.trim()) {
        payload.titleI18n = {
          en: sectionTitleEn.trim() || undefined,
          es: sectionTitleEs.trim() || undefined,
        };
      }
      ...
      setAlertDialog({
        isOpen: true,
        title: "Error",
        message: (e as Error)?.message || "Failed to add section",
        variant: "error",
      });
    }
  };
  ...
  const confirmDeleteSectionAction = async () => {
    ...
    try {
      ...
      setAlertDialog({
        isOpen: true,
        title: "Error",
        message: (e as Error)?.message || "Failed to delete section",
        variant: "error",
      });
    }
  };
  ...
  const openEditField = (field: any, sectionId: string | null) => {
    ...
    const pi = field.placeholderI18n as
      | { en?: string; es?: string }
      | undefined;
    ...
    if (Array.isArray(field.options)) {
      optionsList = field.options.map((o: any) => ({
        ...
        label: (typeof o === "object" && o?.label != null
          ? String(o.label)
          : "") as string,
  ...
  const saveField = async () => {
    ...
    if (fieldForm.type === "select") {
      const list = (fieldForm.optionsList || [])
        ...
        .map((o) => {
          const v =
            (o.value ?? "").trim() ||
            (o.labelEn ?? o.labelEs ?? o.label ?? "").trim();
          const l = (o.label ?? o.labelEn ?? o.labelEs ?? "").trim() || v;
          const opt: any = { value: v, label: l };
          if ((o.labelEn ?? "").trim() || (o.labelEs ?? "").trim())
            opt.labelI18n = {
              en: (o.labelEn ?? "").trim() || undefined,
              es: (o.labelEs ?? "").trim() || undefined,
            };
          return opt;
        });
    ...
    let rules: {
      showWhen?: { field: string; value: boolean | string | number };
    } | null = null;
    if (fieldForm.showWhenField?.trim()) {
      ...
      rules = {
        showWhen: { field: fieldForm.showWhenField.trim(), value: parsed },
      };
    }
    const label =
      fieldForm.label.trim() ||
      fieldForm.labelEn.trim() ||
      fieldForm.labelEs.trim() ||
      "Untitled";
    const payload: any = {
      ...
      placeholder:
        ...
        fieldForm.placeholderEn.trim() ||
        fieldForm.placeholderEs.trim() ||
        undefined,
      helpText:
        fieldForm.helpText.trim() ||
        fieldForm.helpTextEn.trim() ||
        fieldForm.helpTextEs.trim() ||
        undefined,
      ...
      maxFiles:
        fieldForm.type === "image_upload" ? fieldForm.maxFiles : undefined,
      maxSize:
    ...
    if (fieldForm.labelEn.trim() || fieldForm.labelEs.trim())
      payload.labelI18n = {
        en: fieldForm.labelEn.trim() || undefined,
        es: fieldForm.labelEs.trim() || undefined,
      };
    if (fieldForm.placeholderEn.trim() || fieldForm.placeholderEs.trim())
      payload.placeholderI18n = {
        en: fieldForm.placeholderEn.trim() || undefined,
        es: fieldForm.placeholderEs.trim() || undefined,
      };
    if (fieldForm.helpTextEn.trim() || fieldForm.helpTextEs.trim())
      payload.helpTextI18n = {
        en: fieldForm.helpTextEn.trim() || undefined,
        es: fieldForm.helpTextEs.trim() || undefined,
      };
    try {
      if (editingField) {
      ...
      setAlertDialog({
        isOpen: true,
        title: "Error",
        message: (e as Error)?.message || "Failed to save field",
        variant: "error",
      });
    }
  };
  ...
  const confirmDeleteFieldAction = async () => {
    ...
    try {
      ...
      setAlertDialog({
        isOpen: true,
        title: "Error",
        message: (e as Error)?.message || "Failed to delete field",
        variant: "error",
      });
    }
  };
      ...
      >
        ...
        <View style={styles.tabBar}>
          ...
          >
            ...
            <Text
              style={[styles.tabText, tab === "build" && styles.tabTextActive]}
            >
              Build
            </Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text
              style={[
                styles.tabText,
                tab === "preview" && styles.tabTextActive,
              ]}
            >
              Preview
            </Text>
            ...
            <View style={styles.previewToolbar}>
              ...
              <View style={styles.localeToggle}>
                <TouchableOpacity
                  style={[
                    styles.localeBtn,
                    previewLocale === "en" && styles.localeBtnActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      ...
                      previewLocale === "en" && styles.localeBtnTextActive,
                    ]}
                  >
                    EN
                  </Text>
                ...
                <TouchableOpacity
                  style={[
                    styles.localeBtn,
                    previewLocale === "es" && styles.localeBtnActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.localeBtnText,
                      previewLocale === "es" && styles.localeBtnTextActive,
                    ]}
                  >
                    ES
                  </Text>
                  ...
                  >
                    ...
                    <View style={{ marginLeft: "auto" }}>
                      <TouchableOpacity
                        ...
                        activeOpacity={0.7}
                      >
                        <Text style={formBuilder.btnPrimaryText}>
                          Save Form Metadata
                        </Text>
                      </TouchableOpacity>
                    </View>
              ...
              <View style={styles.section}>
                ...
                {(formLevelFields.length > 0 || sections.length > 0) && (
                  <View style={formBuilder.section}>
                    <Text style={formBuilder.sectionTitle}>
                      Form-level fields
                    </Text>
                    {formLevelFields.map((f: any) => (
                      <View key={f.id} style={formBuilder.fieldRow}>
                        ...
                        <TouchableOpacity
                          onPress={() => openEditField(f, null)}
                          style={formBuilder.iconBtn}
                        >
                          <Edit size={16} color="#64748B" />
                        </TouchableOpacity>
                        <TouchableOpacity
                          onPress={() => deleteField(f.id)}
                          style={formBuilder.iconBtn}
                        >
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => openAddField(null)}
                      activeOpacity={0.7}
                    >
                      <Plus size={16} color="#64748B" />
                      <Text style={styles.addFieldTxt}>
                        Add form-level field
                      </Text>
                    </TouchableOpacity>
                  </View>
                )}
                  ...
                  <View key={sec.id} style={formBuilder.section}>
                    ...
                    {(sec.fields ?? []).map((f: any) => (
                      <View key={f.id} style={formBuilder.fieldRow}>
                        ...
                        <Text style={formBuilder.fieldLabel}>{f.label}</Text>
                        <TouchableOpacity
                          onPress={() => openEditField(f, sec.id)}
                          style={formBuilder.iconBtn}
                        >
                          <Edit size={16} color="#64748B" />
                        </TouchableOpacity>
      ...
      <View style={styles.modalOverlay}>
        ...
        >
          ...
          <View
            style={[
              styles.modalActions,
              { borderTopWidth: 0, marginTop: 0, paddingTop: s[4] },
            ]}
          >
            <TouchableOpacity
              style={formBuilder.btn}
              onPress={onClose}
              activeOpacity={0.7}
            >
        ...
        >
          <View style={styles.modalHeader}>
            <H4 style={styles.modalTitle}>
              {isEdit ? "Edit field" : "Add component"}
            </H4>
            <TouchableOpacity
              onPress={onClose}
              style={styles.modalClose}
              ...
              <>
                ...
                <View style={styles.modalRow}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(t) =>
                      setFieldForm((f: any) => ({ ...f, label: t }))
                    }
                    placeholder="Display label"
                  />
                </View>
                <View
                  style={[styles.modalRow, { flexDirection: "row", gap: 12 }]}
                >
                  <View style={{ flex: 1 }}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(t) =>
                        setFieldForm((f: any) => ({ ...f, labelEn: t }))
                      }
                      placeholder="English"
                    />
                  </View>
                  <View style={{ flex: 1 }}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(t) =>
                        setFieldForm((f: any) => ({ ...f, labelEs: t }))
                      }
                      placeholder="Español"
                    />
                  </View>
                ...
                <View style={styles.modalRow}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(t) =>
                      setFieldForm((f: any) => ({ ...f, placeholder: t }))
                    }
                    placeholder="Optional"
                  />
                </View>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: "row", gap: 8 }}>
                      <TextInput
                        ...
                        onChangeText={(t) =>
                          setFieldForm((f: any) => ({ ...f, placeholderEn: t }))
                        }
                        placeholder="EN"
                      />
                      <TextInput
                        ...
                        onChangeText={(t) =>
                          setFieldForm((f: any) => ({ ...f, placeholderEs: t }))
                        }
                        placeholder="ES"
                      />
                    </View>
                ...
                <View style={styles.modalRow}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(t) =>
                      setFieldForm((f: any) => ({ ...f, helpText: t }))
                    }
                    placeholder="Optional"
                  />
                </View>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: "row", gap: 8 }}>
                      <TextInput
                        ...
                        onChangeText={(t) =>
                          setFieldForm((f: any) => ({ ...f, helpTextEn: t }))
                        }
                        placeholder="EN"
                      />
                      <TextInput
                        ...
                        onChangeText={(t) =>
                          setFieldForm((f: any) => ({ ...f, helpTextEs: t }))
                        }
                        placeholder="ES"
                      />
                    </View>
                ...
                >
                  <Switch
                    ...
                    onValueChange={(v) =>
                      setFieldForm((f: any) => ({ ...f, required: v }))
                    }
                    trackColor={{ false: "#E2E8F0", true: "#2563EB" }}
                  />
                ...
                >
                  ...
                  >
                    Show this field only when another field has a specific
                    value.
                  </Text>
                  <View style={styles.modalRow}>
                  ...
                  {fieldForm.showWhenField ? (
                    <View style={styles.modalRow}>
                      ...
                      <TextInput
                        ...
                        onChangeText={(t) =>
                          setFieldForm((f: any) => ({ ...f, showWhenValue: t }))
                        }
                ...
                {fieldForm.type === "select" && (
                  <View style={styles.modalRow}>
                    <Text style={formBuilder.label}>
                      Opciones (Value, Label EN, Label ES)
                    </Text>
                    <Text
                      style={{
                        fontSize: 12,
                    ...
                    >
                      Añade valor y etiquetas en ambos idiomas para cada opción.
                    </Text>
                    {(fieldForm.optionsList || []).map(
                      (
                        opt: {
                          value: string;
                          label: string;
                        ...
                        <View
                          key={idx}
                          style={[formBuilder.optionsRow, { flexWrap: "wrap" }]}
                        >
                          <TextInput
                            style={[
                              formBuilder.input,
                          ...
                          <TouchableOpacity
                            onPress={() =>
                              setFieldForm((f: any) => ({
                                ...f,
                                optionsList: (f.optionsList || []).filter(
                    ...
                    >
                      ...
                      <Text style={formBuilder.btnOutlineText}>
                        Añadir opción
                      </Text>
                    </TouchableOpacity>
                  </View>
                )}
                {(fieldForm.type === "file_upload" ||
