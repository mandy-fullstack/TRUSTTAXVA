...
export const PinChangeModal: React.FC<PinChangeModalProps> = ({ visible, onClose, onSuccess }) => {
    ...
    const handleMainAction = () => {
        switch (step) {
            ...
            case 'new': handleNext(); break;
            case 'confirm': handleSave(); break;
        }
    };
            ...
            <View style={styles.overlay}>
                <View style={styles.modal}>
                    ...
                    <Text style={styles.title}>
                        {step === 'verify' ? t('pin.verify_old_title', 'Verify Current PIN') :
                            step === 'new' ? t('pin.enter_new_title', 'Enter New PIN') :
                                t('pin.confirm_new_title', 'Confirm New PIN')}
                    </Text>

                    <View style={styles.descriptionRow}>
                        <Text style={styles.description}>
                            {step === 'verify' ? t('pin.verify_old_desc', 'Please enter your current PIN to authenticate.') :
                                step === 'new' ? t('pin.enter_new_desc', 'Choose a new 6-digit PIN.') :
                                    t('pin.confirm_new_desc', 'Re-enter your new PIN to confirm.')}
                        </Text>
                    ...
                    <Button
                        ...
                        style={{ marginTop: 16 }}
                    >
                        {isLoading ? t('pin.processing', 'Processing...') : (step === 'confirm' ? t('pin.save_new', 'Save New PIN') : t('pin.continue', 'Continue'))}
                    </Button>
                </View>
            </View>
        </Modal>
