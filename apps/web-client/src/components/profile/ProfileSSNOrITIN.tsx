        ...
        <View style={styles.wrapper}>
            ...
            {showWhy && (
                <View style={styles.whyBox}>
                    ...
                    <Text style={styles.whyBody}>
                        {t('profile.why_ssn_body', 'We use your Social Security Number (SSN) or Individual Taxpayer Identification Number (ITIN) solely for preparing and filing your tax returns with the IRS. This information is required by law for U.S. tax compliance. We encrypt it using AES-256-GCM before storage and never share it with third parties except as required for filing.')}
                    </Text>
                </View>
            )}
            ...
            <View style={styles.typeRow}>
                {(['SSN', 'ITIN'] as const).map((type) => (
                    <TouchableOpacity
                        key={type}
                        style={[styles.typeOption, taxIdType === type && styles.typeOptionActive]}
                        onPress={() => onChangeType(type)}
                        activeOpacity={0.7}
                    >
                        <Text style={[styles.typeText, taxIdType === type && styles.typeTextActive]}>
                            {type}
                        </Text>
                    </TouchableOpacity>
                ))}
