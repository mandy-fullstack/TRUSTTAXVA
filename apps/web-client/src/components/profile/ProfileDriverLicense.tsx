...
interface ProfileDriverLicenseProps {
    ...
    onLoadDecrypted?: () => Promise<{ number: string, stateCode: string, stateName: string, expirationDate: string } | null>; // Función async que carga los datos descifrados completos
}
            ...
            {showWhy && (
                <View style={styles.whyBox}>
                    ...
                    <Text style={styles.whyBody}>
                        {t('profile.why_dl_body', 'We use your driver\'s license details (number, state of issue, expiration) to verify your identity for tax preparation and to comply with IRS due diligence requirements. All data is encrypted with AES-256-GCM before storage. We never display the full number.')}
                    </Text>
                </View>
            )}
            ...
            <View style={styles.row}>
                <View style={[styles.group, { flex: 1 }]}>
                    <MaskedInput
                        ...
                        placeholder={t('profile.dl_number_placeholder', 'Enter license number')}
                        maskType="license"
                        ...
                        onLoadDecrypted={onLoadDecrypted ? async () => {
                            ...
                            if (decrypted) {
                                ...
                                onStateChange((decrypted.stateCode || '').toUpperCase(), (decrypted.stateName || '').toUpperCase());
                ...
                <View style={[styles.group, { flex: 1 }]}>
                    <Text style={styles.sublabel}>{t('profile.dl_state', 'State of issue')}</Text>
                    <TouchableOpacity
                        style={styles.select}
                    ...
                    >
                        <Text style={styles.selectText} numberOfLines={1}>
                            {selectedState ? `${selectedState.name} (${selectedState.isoCode})` : t('profile.select_state', 'Select state')}
                        </Text>
                        ...
                        <View style={styles.dropdown}>
                            ...
                            <ScrollView style={styles.options} keyboardShouldPersistTaps="handled">
                                {filteredStates.map((s) => (
                                    <TouchableOpacity
                                        key={s.isoCode}
                                        style={[styles.opt, stateCode === s.isoCode && styles.optActive]}
                                        onPress={() => {
                                            onStateChange((s.isoCode || '').toUpperCase(), (s.name || '').toUpperCase());
                                            setStateOpen(false);
                                            setStateQuery('');
                                        }}
                                        activeOpacity={0.7}
                                    >
                                        <Text style={styles.optText}>{s.name} ({s.isoCode})</Text>
                                    </TouchableOpacity>
                                ))}
                            </ScrollView>
                        </View>
...
const styles = StyleSheet.create({
    ...
    dropdown: { marginTop: 4, borderWidth: 1, borderColor: '#E2E8F0', backgroundColor: '#FFFFFF', borderRadius: 0, maxHeight: 220 },
    searchInput: {
        height: 40,
    ...
    opt: { paddingVertical: 12, paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: '#F8FAFC' },
