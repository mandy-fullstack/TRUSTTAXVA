...
export function TaxW2ConfirmStep({ data, onChange }: TaxW2ConfirmStepProps) {
  ...
  const setCorrect = (v: boolean) => {
    onChange({
      w2ConfirmCorrect: v,
      w2CorrectionNote: v ? undefined : note || undefined,
    });
  };
    ...
    return (
      <Stack gap="xl">
        ...
        <Text style={styles.desc}>
          {t("tax_wizard.w2_confirm.complete_first")}
        </Text>
      </Stack>
    );
  }
    ...
    <Stack gap="xl">
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>
            {t("tax_wizard.w2_confirm.detected_from")}
          </Text>
        </View>
        <Spacer size="md" />
        {detected ? (
          <View style={styles.rows}>
            <DetectedRow
              label={t("tax_wizard.w2_confirm.taxpayer_name")}
              value={detected.taxpayerName}
            />
            <DetectedRow
              label={t("tax_wizard.w2_confirm.taxpayer_ssn")}
              value={detected.taxpayerSsnMasked}
            />
            <DetectedRow
              label={t("tax_wizard.w2_confirm.address")}
              value={detected.address}
            />
            <DetectedRow
              label={t("tax_wizard.w2_confirm.employer_name")}
              value={detected.employerName}
            />
            <DetectedRow
              label={t("tax_wizard.w2_confirm.wages")}
              value={
                detected.wages != null
                  ? `$${detected.wages.toLocaleString()}`
                  : undefined
              }
            />
            <DetectedRow
              ...
              value={
                detected.federalWithholding != null
                  ? `$${detected.federalWithholding.toLocaleString()}`
                  : undefined
              }
            />
            {detected.socialSecurityWages != null && (
              <DetectedRow
                label={t("tax_wizard.w2_confirm.ss_wages", "SS Wages")}
                value={`$${detected.socialSecurityWages.toLocaleString()}`}
              />
            )}
            {detected.socialSecurityWithheld != null && (
              <DetectedRow
                label={t("tax_wizard.w2_confirm.ss_withheld", "SS Withheld")}
                value={`$${detected.socialSecurityWithheld.toLocaleString()}`}
              />
            )}
            {detected.medicareWages != null && (
              <DetectedRow
                label={t(
                  "tax_wizard.w2_confirm.medicare_wages",
                  "Medicare Wages",
                )}
                value={`$${detected.medicareWages.toLocaleString()}`}
              />
            )}
            {detected.medicareWithheld != null && (
              <DetectedRow
                label={t(
                  "tax_wizard.w2_confirm.medicare_withheld",
                  "Medicare Withheld",
                )}
                value={`$${detected.medicareWithheld.toLocaleString()}`}
              />
            )}
            {detected.stateWages != null && (
              <DetectedRow
                label={t("tax_wizard.w2_confirm.state_wages")}
                value={`$${detected.stateWages.toLocaleString()}`}
              />
            )}
            {detected.stateWithholding != null && (
              <DetectedRow
                label={t("tax_wizard.w2_confirm.state_withholding")}
                value={`$${detected.stateWithholding.toLocaleString()}`}
              />
            )}
          </View>
        ) : (
      ...
      <View style={styles.actions}>
        <Text style={styles.actionLabel}>
          {t("tax_wizard.w2_confirm.is_correct")}
        </Text>
        <View style={styles.buttonRow}>
          ...
          >
            <CheckCircle
              size={18}
              color={confirmCorrect === true ? "#FFF" : "#10B981"}
            />
            <Text
              style={[
                styles.btnText,
                confirmCorrect === true && styles.btnTextActive,
              ]}
            >
              {t("tax_wizard.w2_confirm.correct")}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[
              styles.btn,
              styles.btnOutline,
              confirmCorrect === false && styles.btnWarning,
            ]}
            onPress={() => setCorrect(false)}
          >
            <AlertCircle
              size={18}
              color={confirmCorrect === false ? "#FFF" : "#F59E0B"}
            />
            <Text
              style={[
                styles.btnText,
                confirmCorrect === false && styles.btnTextActive,
              ]}
            >
              {t("tax_wizard.w2_confirm.needs_correction")}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      {confirmCorrect === false && (
        <View style={styles.noteSection}>
          <Text style={styles.noteLabel}>
            {t("tax_wizard.w2_confirm.correction_note")}
          </Text>
          <Spacer size="sm" />
          <TextInput
