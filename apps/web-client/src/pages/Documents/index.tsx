...
export const DocumentsPage = () => {
  ...
  const getGroupDocs = (group: string) => {
    return allDocuments.filter((doc) => {
      ...
      if (group === "LEGAL")
        return type.includes("AGREEMENT") || type.includes("FORM");
      return (
        !type.includes("LICENSE") &&
        !type.includes("PASSPORT") &&
        type !== "SSN" &&
  ...
  const handleDownload = async (doc: any) => {
    try {
      ...
      showAlert({
        title: t("documents.download_error_title", "Error de descarga"),
        message: t(
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showAlert({
        title: t("documents.delete_error_title", "Error al eliminar"),
        message: t(
          "documents.delete_error",
          "No se pudo eliminar el documento de forma segura.",
        ),
        variant: "error",
      });
    } finally {
  ...
  const confirmRename = async (newTitle: string) => {
    if (pendingFile) {
      ...
      try {
        ...
        showAlert({
          title: t("documents.upload_error_title", "Error de carga"),
          message: t(
            "documents.upload_error",
            "No se pudo subir el documento. Revisa tu conexión.",
          ),
          variant: "error",
        });
      } finally {
      ...
      try {
        ...
        showAlert({
          title: t("documents.rename_error_title", "Error al renombrar"),
          message: t(
            "documents.rename_error",
            "No se pudo actualizar el nombre del documento.",
          ),
          variant: "error",
        });
      } finally {
  ...
  const handlePreview = async (doc: any) => {
    try {
      ...
      showAlert({
        title: t("documents.preview_error_title", "Error de vista previa"),
        message: t(
          "documents.preview_error",
  ...
  const FOLDERS = [
    {
      id: "IDENTITY",
      label: t("documents.folders.identity", "Identity Documents"),
      subtitle: "Passport, ID, SSN",
      icon: <User size={isMobile ? 24 : 32} color="#0F172A" />,
    },
    {
      id: "TAX",
      label: t("documents.folders.tax", "Tax Returns & Filings"),
      subtitle: "W-2, 1099, Returns",
      icon: <FileText size={isMobile ? 24 : 32} color="#0F172A" />,
    },
    {
      id: "LEGAL",
      label: t("documents.folders.legal", "Legal & Agreements"),
      subtitle: "Contracts, Forms",
      icon: <Shield size={isMobile ? 24 : 32} color="#0F172A" />,
    },
    {
      id: "OTHER",
      label: t("documents.folders.general", "General & Uploads"),
      subtitle: "Receipts, Other",
      icon: <Folder size={isMobile ? 24 : 32} color="#0F172A" />,
    },
  ];
    ...
    <View style={styles.searchContainer}>
      ...
      >
        ...
        <TextInput
          ...
          placeholder={t(
            "documents.search_placeholder",
            "Search documents, types, or dates...",
          )}
          value={searchQuery}
      ...
      {isSearchFocused && searchQuery.length === 0 && (
        <View style={styles.recommendationPanel}>
          <Text style={styles.recommendationTitle}>
            {t("documents.recently_accessed", "RECENTLY ACCESSED")}
          </Text>
            ...
            >
              ...
              <View style={styles.folderContent}>
                ...
                <Text style={styles.folderSubtitle}>
                  {docs.length} {t("documents.files", "files")}
                </Text>
              </View>
  ...
  const handleDragOver = (e: any) => {
    e.preventDefault();
    if (currentGroup) setIsDragging(true);
  };
  ...
  const renderFiles = () => {
    ...
    return (
      <View
        style={[styles.filesContainer, isDragging && styles.dragActive]}
        {...Platform.select({
          web: {
      ...
      >
        {isDragging && (
          <View style={styles.dropOverlay}>
            <CloudUpload size={64} color="#2563EB" />
            <Text style={styles.dropText}>
              {t("documents.drop_text", "Drop file to upload")}
            </Text>
          </View>
        )}
          ...
          <View style={styles.folderStats}>
            ...
            <View style={styles.statItem}>
              ...
              <Text style={styles.statText}>
                {(folderSize / 1024 / 1024).toFixed(2)} MB Total
              </Text>
            </View>
          </View>
        </View>
        ...
        >
          <View>
            <Text
              style={[styles.folderHeroTitle, isMobile && { fontSize: 20 }]}
            >
              {folderInfo?.label}
            </Text>
            <Text style={styles.folderHeroSubtitle}>
              {folderInfo?.subtitle}
            </Text>
          </View>
          <Button
        ...
        {groupDocs.length === 0 ? (
          ...
          >
            ...
            <Text style={styles.emptyStateSubtitle}>
              Click or Drag to upload your first{" "}
              {folderInfo?.label.toLowerCase()}
            </Text>
          </TouchableOpacity>
        ) : (
          <View style={[styles.grid, { gap }]}>
            {groupDocs.map((doc) => (
              <View
                key={doc.id}
                style={[styles.fileCard, { width: cardWidth as any }]}
              >
  ...
  return (
    <Layout>
      ...
      <View style={[styles.container, isMobile && { padding: 16 }]}>
        <View style={styles.mainHeader}>
          <View>
            <Text
              variant="h2"
              style={[styles.pageTitle, isMobile && { fontSize: 24 }]}
            >
              {t("header.documents", "Vault")}
            </Text>
            <Text style={styles.pageSubtitle}>
              {t(
                "documents.vault_subtitle",
                "Secured document management with end-to-end encryption",
              )}
            </Text>
          </View>
          {!isMobile && (
            <View style={styles.vaultStats}>
              ...
              <Text style={styles.vaultStatsText}>
                {t("documents.encrypted_store", "ENCRYPTED STORE")}
              </Text>
            </View>
          )}
        </View>
      ...
      <ConfirmDialog
        ...
        message={t("documents.delete_confirm_msg", {
      ...
      <RenameModal
        ...
        title={
          pendingFile
            ? t("documents.confirm_name", "Confirm Document Name")
            : t("documents.rename_title", "Rename Document")
        }
      />
      ...
      <ConfirmDialog
        ...
        message={t(
          "documents.upload_success_msg",
          "Your document has been successfully uploaded and securely encrypted.",
        )}
...
const styles = StyleSheet.create({
  ...
  filesContainer: { position: "relative", minHeight: 400 },
  dragActive: {
