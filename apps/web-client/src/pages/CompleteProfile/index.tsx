...
const CompleteProfilePage: React.FC = () => {
        ...
        const checkProfile = async () => {
            try {
                ...
                const initialData = {
                    ...
                    dateOfBirth: me.dateOfBirth ? new Date(me.dateOfBirth).toISOString().split('T')[0] : '',
    ...
    const handleSubmit = async (data: any) => {
        ...
        const finalData: any = {};
        const validKeys = [
            'firstName', 'middleName', 'lastName', 'dateOfBirth', 'countryOfBirth',
            'primaryLanguage', 'taxIdType', 'ssn',
            'driverLicenseNumber', 'driverLicenseStateCode', 'driverLicenseIssueDate', 'driverLicenseExpiration',
            'passportNumber', 'passportCountryOfIssue', 'passportIssueDate', 'passportExpiration',
            'acceptTerms'
        ];

        validKeys.forEach(key => {
            if (rawData[key] !== '' && rawData[key] !== undefined && rawData[key] !== null) {
        ...
        if (finalData.taxIdType === 'PASSPORT' || (formData.idType === 'PASSPORT' && !finalData.idType)) {
        ...
        else if (finalData.taxIdType === 'DL' || (formData.idType === 'DL' && !finalData.idType)) {
        ...
        case 1:
            return (
                <CompleteProfileLayout
                    ...
                    title="IDENTIFY"
                    subtitle="ESTABLISH YOUR OFFICIAL LEGAL NOMENCLATURE."
                >
        ...
        case 2:
            return (
                <CompleteProfileLayout
                    ...
                    totalSteps={TOTAL_STEPS}
                    title="BIRTH"
                    subtitle="CHRONOLOGICAL VERIFICATION FOR IRS COMPLIANCE."
                >
                    <Step1bBirth onNext={handleNext} onBack={handleBack} initialData={formData} />
                </CompleteProfileLayout>
            );
        case 3:
            return (
                <CompleteProfileLayout
                    ...
                    totalSteps={TOTAL_STEPS}
                    title="ORIGIN"
                    subtitle="GEOGRAPHICAL AND LINGUISTIC BASELINE."
                >
                    <Step1cOrigin onNext={handleNext} onBack={handleBack} initialData={formData} />
                </CompleteProfileLayout>
            );
        case 4:
            return (
                <CompleteProfileLayout
                    ...
                    totalSteps={TOTAL_STEPS}
                    title="SECURITY"
                    subtitle="ENCRYPTED TAX IDENTIFICATION PROTOCOL."
                >
                    <Step2Security onNext={handleNext} onBack={handleBack} initialData={formData} />
                </CompleteProfileLayout>
            );
        case 5:
            return (
                ...
                >
                    <Step3DocumentType onNext={handleNext} onBack={handleBack} initialData={formData} />
                </CompleteProfileLayout>
            );
        case 6:
            ...
            return (
                <CompleteProfileLayout
                    currentStep={6}
                    totalSteps={TOTAL_STEPS}
                    title={isPassport ? "PASSPORT" : "LICENSE"}
                    subtitle="SECURE ENCRYPTED UPLOAD & VERIFICATION."
                >
                    {isPassport ? (
                        <Step3Passport onNext={handleNext} onBack={handleBack} initialData={formData} />
                    ) : (
                        <Step3DriverLicense onNext={handleNext} onBack={handleBack} initialData={formData} />
                    )}
                </CompleteProfileLayout>
            );
        case 7:
            return (
                <CompleteProfileLayout
                    ...
                    totalSteps={TOTAL_STEPS}
                    title="PROTOCOL"
                    subtitle="FINAL DEPLOYMENT AND SECURE ACTIVATION."
                >
                    <Step4Authorization onSubmit={handleSubmit} onBack={handleBack} isSubmitting={isSubmitting} />
                </CompleteProfileLayout>
            );
        default:
            return null;
