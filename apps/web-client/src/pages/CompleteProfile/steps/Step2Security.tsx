...
export const Step2Security: React.FC<Step2Props> = ({ onNext, onBack, initialData }) => {
    ...
    React.useEffect(() => {
        if (initialData?.taxIdType !== undefined) setTaxIdType(initialData.taxIdType);
        ...
        <View style={styles.container}>
            <View style={styles.selector}>
                {(['SSN', 'ITIN'] as const).map((type) => (
                    ...
                    >
                        <Text style={[styles.optText, taxIdType === type && styles.optTextActive]}>{type}</Text>
                    </TouchableOpacity>
                ))}
            </View>
            ...
            <View style={styles.group}>
                <Input
                    label={`${taxIdType} IDENTIFICATION NUMBER`}
                    value={ssn}
                ...
                {error ? (
                    ...
                    <Text style={styles.info}>AES-256-GCM ENCRYPTION ACTIVE. PREVIOUS DATA HYDRATED.</Text>
                )}
            </View>
