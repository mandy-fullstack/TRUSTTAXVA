    ...
    const initialize = async () => {
      try {
        ...
        if (
          state?.orderId &&
          !paramId &&
          processingOrderRef.current !== state.orderId
        ) {
          processingOrderRef.current = state.orderId;
          const subject = `Orden #${state.displayId || state.orderId}`;
          // Check if conversation already exists for this order
          const existing = data.find((c) => c.subject === subject);
          if (existing) {
            navigate(`/dashboard/chat/${existing.id}`, { replace: true });
          } else {
            // Create new conversation automatically
            const newConv = await api.createConversation(subject);
            setConversations((prev) => [newConv, ...prev]);
            navigate(`/dashboard/chat/${newConv.id}`, { replace: true });
          }
        }
      } catch (error) {
  ...
  return (
    <Layout noScroll={true}>
      <View style={styles.container}>
        ...
        {/* Chat Details Area */}
        <View
          style={[styles.chatArea, !paramId && styles.chatAreaHiddenOnMobile]}
        >
          {paramId ? (
            <>
              <View style={styles.chatHeader}>
                <TouchableOpacity
                  ...
                  onPress={() => navigate("/dashboard/chat")}
                >
                ...
                <View>
                  ...
                  >
                    <Text style={styles.chatHeaderTitle}>
                      {currentConversation?.subject || "Chat"}
                    </Text>
                    <View
                      style={{
                        width: 8,
